...
def Hora_to_Float(hora):
    ...
    """
    try:
        parte_inteira, parte_decimal = hora.split(":")
...
def validar_datas_e_calcular_horas1(data1_str, data2_str):
    """
...
def Salva_Planilha1(dados, resultado):
...
def VerificaVazio(dados) -> int:
    ...
    if dados['pi_memb_1'] == 0.0:        # Membrana PI #1 0.000
        ...
        erro =  6  
    elif dados['fli_memb_1'] == '':     # Membrana FI #1 MM:SS 
        erro =  7
    elif dados['fli_memb_2'] == '':     # Membrana FI #2 MM:SS
        erro =  8
    elif dados['fli_memb_3'] == '':     # Membrana FI #3 MM:SS
        erro =  9 
        ...
        erro =  16 
    elif dados['prd_res1'] == 0.0:      # PRD Resultado #1 0.0
        erro =  17 
    elif dados['prd_res2'] == 0.0:      # PRD Resultado #2 0.0
        erro =  18
    elif dados['prd_res3'] == 0.0:      # PRD Resultado #3 0.0
        erro =  19
    elif dados['pb_refproduto'] == 0.0:  # PB Referencial (psi) 
        erro =  20
    elif dados['flf_memb_1'] == 0.00:      # Fluxo Final #1 0.00
        erro =  21 
    elif dados['flf_memb_2'] == 0.00:      # Fluxo Final #2 0.00
        erro =  22
    elif dados['flf_memb_3'] == 0.00:      # Fluxo Final #3 0.00
        erro =  23 
    elif dados['dis_res1'] == 0.0:      # Resultado #1 0.0
        erro =  24                                                        
    elif dados['dis_res2'] == 0.0:      # Resultado #2 0.0
        erro =  25  
...
def Planilha(db, planilha_nova):
    ...
    with container1:
        ...
        with col1:
            local_teste  = st.text_input('Local de Teste', max_chars= 12, value=db['local_teste'])
            pessoa_local = st.text_input('Pessoa Local', max_chars= 12, value=db['pessoa_local'])
            id_local     = st.text_input('ID do Local', max_chars= 12, value=db['id_local'])
            db['local_teste']= local_teste
            db['pessoa_local']= pessoa_local
            db['id_local']= id_local
        with col2:   
            if planilha_nova:
                dt_chegada = st.text_input('Data de Chegada',placeholder='DD-MM-AAAA')
                hr_chegada = st.text_input('Hora de Chegada',placeholder='HH:MM')
            else:
                dt_chegada = st.text_input('Data de Chegada',value=db['dt_chegada'])
                hr_chegada = st.text_input('Hora de Chegada',value=Hora_to_Float(db['hr_chegada']))   
                print('passei por aqui',Hora_to_Float(db['hr_chegada']),hr_chegada )

            try:
                db['dt_chegada']= dt_chegada.strftime("%Y-%m-%d")
                db['hr_chegada']= Hora_to_Float(hr_chegada) # .strftime('%H:%M')
            except:
                db['dt_chegada']= hoje.strftime('%Y-%m-%d')
                db['hr_chegada']= hoje.strftime('%H:%M')
    ...
    with container2:
        ...
        with col1:
            linha  = st.text_input('Linha do filtro', max_chars= 40, value=db['linha'])
            cat_membr = st.text_input('Nº Catálogo da Membrana',max_chars= 40, value=db['cat_membr'])
            db['linha']= linha
            db['cat_membr']= cat_membr
        with col2:   
            fabricante = st.text_input('Fabricante do filtro',max_chars= 40, value=db['fabricante']) 
            poro_cat_membr= st.text_input('Poro', max_chars= 12, value=db['poro_cat_membr'])
            db['fabricante']= fabricante
            db['poro_cat_membr']= poro_cat_membr
        col1, col2, col3 = st.columns(3)
        with col1:
            temp_filtra = st.text_input('Temperatura de Filtração (°C)', max_chars= 12, value=db['temp_filtra'])
            tara  = st.text_input('Tara da Balança', max_chars= 12, value=db['tara'])
            produto = st.text_input('Produto', max_chars= 20, value=db['produto'])
            db['temp_filtra']= temp_filtra
            db['tara']= tara
            db['produto']= produto
        with col2:   
            tmp_contato = st.text_input('Tempo de contato (horas)', max_chars= 10, value=db['tmp_contato'])  
            tempera_local = st.text_input('Temperatura Local (°C)',max_chars= 12, value=db['tempera_local'])
            lote = st.text_input('Lote da membrana', max_chars= 12, value=db['lote']) 
            db['tmp_contato']= tmp_contato
            db['tempera_local']= tempera_local
            db['lote']= lote
        with col3:  
            armaz = st.text_input('Armazenagem local',max_chars= 20, value=db['armaz'])  
            umidade = st.text_input('Umidade',max_chars= 20, value=db['umidade']) 
            volume = st.text_input('Volume', max_chars= 12, value=db['volume'])
            db['armaz']=armaz
            db['umidade']=umidade
            db['volume']=volume
    ...
    with container3:
        col1, col2, col3 = st.columns(3)   
        with col1:  
            lote1 = st.text_input('Lote #1', max_chars= 10, value=db['lote1']) 
            cat_disp = st.text_input('Catálogo do Dispositivo', max_chars= 12, value=db['cat_disp']) 
            db['lote1']= lote1
            db['cat_disp']= cat_disp
        with col2:  
            lote2 = st.text_input('Lote #2', max_chars= 10, value=db['lote2']) 
            lote_disp = st.text_input('Lote do Dispositivo', max_chars= 10, value=db['lote_disp'])
            db['lote2']= lote2
            db['lote_disp']= lote_disp
        with col3:  
            lote3 = st.text_input('Lote #3', max_chars= 10, value=db['lote3']) 
            serial_cat_disp = st.text_input('Serial Dispositivo', max_chars= 6, value=db['serial_cat_disp'])
            db['lote3']= lote3
            db['serial_cat_disp']= serial_cat_disp
    ...
    with container4:
        ...
        with col1:
            st.markdown('<div style="text-align: center;"><h6>Pesagem Inicial(g)</h6></div>', unsafe_allow_html=True)
        with col2:
            pi_memb_1 = st.number_input('Membrana PI #1', format=format_3casas, value=db['pi_memb_1'])   
            pi_memb_2 = st.number_input('Membrana PI #2', format=format_3casas, value=db['pi_memb_2']) 
            pi_memb_3 = st.number_input('Membrana PI #3', format=format_3casas, value=db['pi_memb_3'])
            db['pi_memb_1']= pi_memb_1      # Membrana Inicial 1 0.000
            db['pi_memb_2']= pi_memb_2      # Membrana Inicial 2
            db['pi_memb_3']= pi_memb_3      # Membrana Inicial 3
        ...
        with col4:   
            if planilha_nova:
                fli_memb_1 = st.text_input('Membrana FI #1', max_chars= 5, placeholder='MM:SS')
                fli_memb_2 = st.text_input('Membrana FI #2', max_chars= 5, placeholder='MM:SS')
                fli_memb_3 = st.text_input('Membrana FI #3', max_chars= 5, placeholder='MM:SS')
            else:
                fli_memb_1 = st.text_input('Membrana FI #1', max_chars= 5, value=db['fli_memb_1'])
                fli_memb_2 = st.text_input('Membrana FI #2', max_chars= 5, value=db['fli_memb_2'])
                fli_memb_3 = st.text_input('Membrana FI #3', max_chars= 5, value=db['fli_memb_3'])   
            db['fli_memb_1']= string_para_float(fli_memb_1)    # Membr 1 Inic - 100 ml 0.00
            db['fli_memb_2']= string_para_float(fli_memb_2)    # Membr 2 Inic - 100 ml 0.00
            db['fli_memb_3']= string_para_float(fli_memb_3)    # Membr 3 Inic - 100 ml 0.00
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            pb_padraowfi = st.number_input('PB Padrão Fluido Padrão (psi)', format=format_1casa, step=0.1, value=db['pb_padraowfi'])
            db['pb_padraowfi']= pb_padraowfi
        with col2:
            wfi_res1 = st.number_input('Fluido Padrão Resultado #1', format=format_1casa, step=0.1, value=db['wfi_res1'])  
            wfi_res2 = st.number_input('Fluido Padrão Resultado #2', format=format_1casa, step=0.1, value=db['wfi_res2'])
            wfi_res3 = st.number_input('Fluido Padrão Resultado #3', format=format_1casa, step=0.1, value=db['wfi_res3'])
            db['wfi_res1']= wfi_res1
            db['wfi_res2']= wfi_res2
            db['wfi_res3']= wfi_res3
        with col3:
            wfi_id1 = st.text_input('Fluido Padrão ID #1', max_chars= 20, value=db['wfi_id1'])  
            wfi_id2 = st.text_input('Fluido Padrão ID #2', max_chars= 20, value=db['wfi_id2'])
            wfi_id3 = st.text_input('Fluido Padrão ID #3', max_chars= 20, value=db['wfi_id3'])
            db['wfi_id1']= wfi_id1
            db['wfi_id2']= wfi_id2
            db['wfi_id3']= wfi_id3   
        ...
        try:
            db['dt_wfi']= dt_wfi.strftime("%Y-%m-%d")
            db['hr_wfi']= hr_wfi.strftime('%H:%M')
        except:
            db['dt_wfi']= hoje.strftime('%Y-%m-%d')
            db['hr_wfi']= hoje.strftime('%H:%M')    
        # with col3:
    ...
    with container5:
        ...
        with col3:
            ...
            horas_contato = validar_datas_e_calcular_horas1(data1, data2)
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            pb_refproduto = st.number_input('PB Referencial (psi)', format=format_1casa, value=db['pb_refproduto'], 
                                            help= 'Usar teste Referencial', step=0.1)
            db['pb_refproduto']=pb_refproduto
        with col2:
            prd_res1 = st.number_input('PRD Resultado #1', format=format_1casa, step=0.1, value=db['prd_res1'])  
            prd_res2 = st.number_input('PRD Resultado #2', format=format_1casa, step=0.1, value=db['prd_res2'])
            prd_res3 = st.number_input('PRD Resultado #3', format=format_1casa, step=0.1, value=db['prd_res3'])
            db['prd_res1']= prd_res1
            db['prd_res2']= prd_res2
            db['prd_res3']= prd_res3
        with col3:
            prd_id1 = st.text_input('PRD ID #1', max_chars= 20, value=db['prd_id1'])  
            prd_id2 = st.text_input('PRD ID #2', max_chars= 20, value=db['prd_id2'])
            prd_id3 = st.text_input('PRD ID #3', max_chars= 20, value=db['prd_id3']) 
            db['prd_id1']= prd_id1
            db['prd_id2']= prd_id2
            db['prd_id3']= prd_id3  
    ...
    with container6:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            flf_memb_1 = st.text_input('Fluxo Final #1', max_chars= 5, placeholder='mm:ss', value=db['flf_memb_1'])
            db['flf_memb_1']= string_para_float(flf_memb_1)    # Membr 1 Final - 100 ml 0.00
        with col2:   
            flf_memb_2 = st.text_input('Fluxo Final #2', max_chars= 5, placeholder='mm:ss', value=db['flf_memb_2'])
            db['flf_memb_2']= string_para_float(flf_memb_2)    # Membr 2 Final - 100 ml 0.00
        with col3: 
            flf_memb_3 = st.text_input('Fluxo Final #3', max_chars= 5, placeholder='mm:ss', value=db['flf_memb_3'])
            db['flf_memb_3']= string_para_float(flf_memb_3)    # Membr 3 Final - 100 ml 0.00 
    ...
    with container7:
        ...
        with col2:
            wfif_res1 = st.number_input('Fluido Padrão final Resultado #1', format=format_1casa, step=0.1, value=db['wfif_res1'])  
            wfif_res2 = st.number_input('Fluido Padrão final Resultado #2', format=format_1casa, step=0.1, value=db['wfif_res2'])
            wfif_res3 = st.number_input('Fluido Padrão final Resultado #3', format=format_1casa, step=0.1, value=db['wfif_res3'])
            db['wfif_res1']= wfif_res1
            db['wfif_res2']= wfif_res2
            db['wfif_res3']= wfif_res3
        with col3:
            wfif_id1 = st.text_input('Fluido Padrão final ID #1', max_chars= 20, value=db['wfif_id1'])  
            wfif_id2 = st.text_input('Fluido Padrão final ID #2', max_chars= 20, value=db['wfif_id2'])
            wfif_id3 = st.text_input('Fluido Padrão final ID #3', max_chars= 20, value=db['wfif_id3'])  
            db['wfif_id1']= wfif_id1
            db['wfif_id2']= wfif_id2
            db['wfif_id3']= wfif_id3 
    ...
    with container8:
        ...
        col1, col2, col3 = st.columns(3)
        with col1: 
            pf_memb_1 = st.number_input('Peso Final #1', format=format_3casas, step=0.01, value=db['pf_memb_1'])
            db['pf_memb_1']= pf_memb_1      # Membrana Final 1 0.000 
        with col2:   
            pf_memb_2 = st.number_input('Peso Final #2', format=format_3casas, step=0.01, value=db['pf_memb_2']) 
            db['pf_memb_2']= pf_memb_2      # Membrana Final 2
        with col3: 
            pf_memb_3 = st.number_input('Peso Final #3', format=format_3casas, step=0.01, value=db['pf_memb_3'])
            db['pf_memb_3']= pf_memb_3      # Membrana Final 3
    ...
    with container9:
        ...
        with col2:
            dis_res1 = st.number_input('Resultado PRD#1', format=format_1casa, step=0.1, value=db['dis_res1'])  
            dis_res2 = st.number_input('Resultado PRD#2', format=format_1casa, step=0.1, value=db['dis_res2'])
            db['dis_res1']= dis_res1
            db['dis_res2']= dis_res2
        with col3:
            dis_id1 = st.text_input('ID #1', max_chars= 20, value=db['dis_id1'])  
            dis_id2 = st.text_input('ID #2', max_chars= 20, value=db['dis_id2'])
            db['dis_id1']= dis_id1
            db['dis_id2']= dis_id2
    ...
    with container10:
        ...
        col1, col2 = st.columns(2)
        with col1:
            crit_var_peso = st.number_input('Critério Variação Peso <= (%)', format=format_1casa, step=0.1, value=db['crit_var_peso'])
            db['crit_var_peso']= crit_var_peso      # Critério de avaliação % Variação Peso 
        with col2:   
            crit_var_vazao = st.number_input('Critério Variação Vazão <= (%)', format=format_1casa, step=0.1, value=db['crit_var_vazao'])
            db['crit_var_vazao']= crit_var_vazao    # Critério de avaliação % Variação Vazão
    ...
    if st.button('Verificar resultados', type='primary'):
        fim_planilha = True
        erro = VerificaVazio(db)
        if erro == 0:
            return db, fim_planilha
