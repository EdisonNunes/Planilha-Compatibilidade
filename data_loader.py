...
# -------------------------------------------------------------------------------------------
# create table public.sasdatanew (
#   id serial not null,
#   relatorio text null,
#   status_rel_01 text null,
#   dt_agendada_01 text null,
...
#   pb_estimado text null,
#   conclusao text null,
#   constraint sasdatanew_pkey primary key (id)
# ) TABLESPACE pg_default;
...
def listar_registros(filtro_relatorio=""):
    # query = supabase.table("sasdatanew").select("id, relatorio, cliente, finalizado, status_rel_01")
    query = supabase.table("sasdatanew").select("id, relatorio, cliente, status_rel_01")
    if filtro_relatorio:
...
def listar_todos_registros():
    return supabase.table("sasdatanew").select("*").order("relatorio", desc=False).execute().data

def incluir_registro(dados):
    # dados["id"] = str(uuid.uuid4())
    supabase.table("sasdatanew").insert(dados).execute()

def alterar_registro(id, dados):
    supabase.table("sasdatanew").update(dados).eq("id", id).execute()

def excluir_registro(id):
    supabase.table("sasdatanew").delete().eq("id", id).execute()
